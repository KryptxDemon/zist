...
import { themeService, vocabService, quoteService } from '@/services/mediaService';
...
export default function Feed() {
  ...
  async function loadPosts() {
    setIsLoading(true);
    try {
      const data = await feedService.getPosts(filter === 'all' ? undefined : filter);
      setPosts(data);
    } catch (error) {
      ...
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in pb-20 md:pb-0">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Share and discover learning insights</p>
          </div>
          <ComposeDialog
            open={isComposeOpen}
...
function PostCard({ post, onLike, onSave }: {
  ...
  const typeConfig = {
    ...
    vocab: { icon: BookOpen, color: 'text-emerald-400', bg: 'bg-emerald-500/20' },
  ...
  return (
    <div className="glass grain rounded-2xl p-5 animate-fade-in">
      ...
      <div className="bg-accent/30 rounded-xl p-4 mb-4">
        {post.type === 'theme' && (
          <div>
            <h4 className="font-display font-semibold text-foreground">{(post.content as ThemeConcept).title}</h4>
            {(post.content as ThemeConcept).summary && (
              <p className="text-sm text-muted-foreground mt-1">{(post.content as ThemeConcept).summary}</p>
            )}
          </div>
        )}
        {post.type === 'vocab' && (
          <div>
            <h4 className="font-display font-semibold text-foreground">{(post.content as VocabItem).word}</h4>
            {(post.content as VocabItem).definition && (
              <p className="text-sm text-muted-foreground mt-1">{(post.content as VocabItem).definition}</p>
            )}
          </div>
        )}
        {post.type === 'quote' && (
          <div>
            <p className="text-foreground italic">"{(post.content as QuoteItem).text}"</p>
            {(post.content as QuoteItem).speaker && (
              <p className="text-sm text-muted-foreground mt-2">— {(post.content as QuoteItem).speaker}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className={cn(
            ...
            post.isLiked ? 'text-rose-400' : 'text-muted-foreground hover:text-foreground'
          )}
        >
...
function ComposeDialog({ open, onOpenChange, onPost, userName }: {
  open: boolean;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="space-y-2">
            <Label>{postType === 'quote' ? 'Quote Text' : postType === 'vocab' ? 'Word' : 'Theme Title'}</Label>
            <Input
              placeholder={postType === 'quote' ? 'Enter the quote...' : postType === 'vocab' ? 'Enter the word...' : 'Enter theme title...'}
              value={content.title}
          ...
          <div className="space-y-2">
            <Label>{postType === 'quote' ? 'Meaning' : postType === 'vocab' ? 'Definition' : 'Summary'}</Label>
            <Textarea
              placeholder={postType === 'quote' ? 'What does it mean to you?' : postType === 'vocab' ? 'Enter definition...' : 'Brief summary...'}
