...
const MISSING_CREATOR = "Creator not listed";
const MISSING_AUTHOR = "Author not listed";
const MISSING_DESCRIPTION =
  "No description available for this title yet. You can add your own notes after saving.";

function getMissingCreator(isBook: boolean): string {
  return isBook ? MISSING_AUTHOR : MISSING_CREATOR;
}

function getResultTags(item: SearchResultItem): string[] {
  const tags: string[] = [];
...
const fallbackCovers: Record<MediaType, string> = {
  movie:
...
function getResultCover(
  item: Pick<SearchResultItem, "type" | "cover_url">,
): string {
